...
html = r"""<title>Tri &amp; Half Rotation</title>
...
<div class="wrap">
  ...
  <footer>Built for Theo · rotate continuously · tap a session to check it off, synced across your devices.</footer>
</div>
...
(function(){
  ...
  let unsub = null;
  function subscribeWeek(n){
    if (!db) { renderWeek(); return; }
    if (unsub) { unsub(); unsub = null; }
    const ref = db.doc('checks/w' + n);
    unsub = ref.onSnapshot(snap => {
      checks['w' + n] = snap.exists ? (snap.data() || {}) : {};
      renderWeek();
      renderCoach();
    }, () => { /* degrade silently to local-only */ });
  }

  async function persistWeek(n){
    if (!db) return;
    try {
      const ref = db.doc('checks/w' + n);
      await ref.set(checks['w' + n] || {});
    } catch (e) { /* offline / not granted — local state still reflects the tap */ }
  }
  ...
  async function persistSettings(){
    if (!db) return;
    try { await db.doc('settings/config').set(settings); } catch (e) { /* offline */ }
  }

  function subscribeSettings(){
    if (!db) return;
    db.doc('settings/config').onSnapshot(snap => {
      if (snap.exists) settings = Object.assign({ eventEnabled:false, eventDate:null, eventType:'hm', manualStart:null }, snap.data());
      renderCoach(); renderWeek(); buildRail();
  ...
  async function init(){
    ...
    if (db) { subscribeWeek(currentWeek); subscribeSettings(); }
  }
  init();
})();
</script>
