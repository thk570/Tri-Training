...
<div class="wrap">
  ...
  <footer>Built for Theo · rotate continuously · tap a session to check it off — saved on this device/browser only (this is a standalone file, so nothing syncs between phone and desktop).</footer>
</div>
...
(function(){
  ...
  const CHECKS_KEY = 'triplan-checks-v2';
  const SETTINGS_KEY = 'triplan-settings-v1';
  ...
  function saveChecksToStorage(){
    try { localStorage.setItem(CHECKS_KEY, JSON.stringify(checks)); }
    catch (e) { /* storage unavailable (private browsing etc) — state still works this session */ }
  }
...
with open("tri-half-rotation-plan.html", "w") as f:
    f.write(html)
